...
_tts_md_filtered = [tt_md_TokenStartBlockElement,
...
# whitespace tokens
_tts_md_whitespace = [tt_md_TokenSpace,
                      tt_md_TokenTab,
                      tt_md_TokenNewline,
                      tt_md_TokenNewlines,
...
_tts_md_newlines_single = [tt_md_TokenNewline,
                           tt_md_TokenNewlineBR,
                           ]

_tts_md_newlines_all = [tt_md_TokenNewline,
                        tt_md_TokenNewlineBR,
                        tt_md_TokenNewlines,
                        ]

_tts_md_startblocks = [tt_md_TokenStartBlockquote,
                       tt_md_TokenStartBlockElement,
                       ]
_tts_md_endblocks = [tt_md_TokenEndBlockquote,
                     tt_md_TokenEndBlockElement,
                     ]

_tts_md_blocksall = _tts_md_startblocks + _tts_md_endblocks

_tts_md_blockquotes = [tt_md_TokenStartBlockquote,
                       tt_md_TokenEndBlockquote,
                       ]

_tts_md_newlined_text_start = [tt_md_TokenHR,
                               tt_md_TokenHNStart,
                               tt_md_TokenLiStart,
                               ]
...
_tts_md_code = [tt_md_TokenStartCode,
                tt_md_TokenEndCode,
                ]
...
def cleanup_space_backwards(stack, newlines_ensure=0, newline_blockquote=None, newline_codeblock=None, dbg=False):
    """
                ...
                if _md_type in _tts_md_newlines_all:
                    ...
                    if (newline_prefix != _lt_prefix):
                        ...
                        if _md_type == tt_md_TokenNewlines:
                            ...
                            if _lt2 and _lt2.get('_md_type') in _tts_md_newlined_text_end:
                                newlines_ensure = 0
                                _tok = _contextual_TokenNewlines(newlines=2, blockquoted=_lt_blockquoted, codeblocked=_lt_codeblocked, )
                            else:
                                _tok = _contextual_TokenNewlines(newlines=1, blockquoted=_lt_blockquoted, codeblocked=_lt_codeblocked, )
                            stack.append(_tok)
                        break
                _discarded = stack.pop()
            else:
    ...
    if newlines_ensure:
        ...
        if newlines_ensure:
            ...
            _tok = _contextual_TokenNewlines(newlines_ensure, blockquoted=_blockquoted, codeblocked=_codeblocked, )
            stack.append(_tok)
...
def to_markdown(
    ...
    allowed_tags_blocks = MARKDOWN_TAGS_PASSTHROUGH_BLOCKS if allowed_tags_blocks is None else allowed_tags_blocks
    allowed_tags_attributes = MARKDOWN_TAGS_ATTRIBUTES if allowed_tags_attributes is None else allowed_tags_attributes
    ...
    character_italicbold = character_italicbold if character_italicbold in ('*', '_') else '*'
    character_unordered_listitem = character_unordered_listitem if character_unordered_listitem in ('*', '-', '+') else '*'
    ...
    def _process_token_idx(token_idx):
        ...
        token_next = list_walker[_token_idx__next] if _token_idx__next < len_walker else None
        token_next1 = list_walker[_token_idx__next1] if _token_idx__next1 < len_walker else None
        ...
        if ttype in (tt_StartTag, tt_EndTag):
            ...
            if ttype == tt_StartTag:
                if token_next and (token_next.get('type') == 'EndTag') and (token_next.get('name') == name):
                    return None
            elif ttype == tt_EndTag:
                token_prev = list_walker[token_idx - 1] if token_idx > 0 else None
                if token_prev and (token_prev.get('type') == 'StartTag') and (token_prev.get('name') == name):
                    return None
            # end removal
            ...
            if name in ('p', 'div', ):
                ...
                if ttype == tt_StartTag:
                    ...
                    if _render_tag:
                        _in['a__tag'] += 1
                        return clean_token_attributes(token, 'a', allowed_tags_attributes)
                    else:
                        _href = None
                        # _link_text
                        _title = None
                        ...
                        return TokenAMarkdown(_href, _link_text, title=_title, reference=_reference)
                ...
                if ttype == tt_StartTag:
                    _in['blockquote'] += 1
                    return (TokenStartBlockElement('blockquote'),
                            TokenStartBlockquote(_in['blockquote']),
                            )
                elif ttype == tt_EndTag:
                    _in['blockquote'] -= 1
                    return (TokenEndBlockquote(_in['blockquote']),
            ...
            if name == 'img':
                # note: img
                if img_as_tag:
                    token = clean_token_attributes(token, 'img', allowed_tags_attributes)
                    return token
                else:
                    _href = None
                    _alt = None
                    ...
                    return TokenImgMarkdown(_href, alt=_alt, title=_title, reference=_reference)
            ...
            if _last_sig_md in _tts_md_startblocks:  # (tt_md_TokenStartBlockElement, tt_md_TokenStartBlockquote)
    ...
    if is_fragment:
        if token_stack:
            _tok = token_stack[0]
            if ((_tok.get('name') == FRAGMENT_TYPE)
                 and
    ...
    for (token_idx, token) in enumerate(token_stack):
        ...
        if _t_md in _tts_md_startblocks:  # (tt_md_TokenStartBlockquote, tt_md_TokenStartBlockElement, )
            ...
            token_stack__post = cleanup_space_backwards(token_stack__post, newlines_ensure=_newlines_ensure, newline_blockquote=_newline_blockquote, dbg=True)

        elif _t_md in _tts_md_endblocks:  # (tt_md_TokenEndBlockquote, tt_md_TokenEndBlockElement, )
            # if we're going block-to-block, skip to the next block
            ...
            _newline_blockquote = _t_md_blockquote if _newlines_ensure else None
            token_stack__post = cleanup_space_backwards(token_stack__post, newlines_ensure=_newlines_ensure, newline_blockquote=_newline_blockquote)
            continue
                ...
                if _last_sig_md:
                    ...
                    if _t_md in _tts_md_newlines_all:
                        if _last_sig_md in _tts_md_newlines_all:
                            ...
                            _tok = _contextual_TokenNewlines(newlines=2, blockquoted=None, codeblocked=None, )
                            ...
                            if _t_next_md in _tts_md_startblocks:
                                ...
                                _tok = _contextual_TokenNewlines(newlines=1, blockquoted=None, codeblocked=None, )
                                token_stack__post.append(_tok)
                                continue
            ...
            if _t_md == tt_md_TokenStartCode:
                if _render_code_as_block_backwards(token_stack__post) and _render_code_as_block_frontwards(token_stack, token_idx):
                    # token['type'] = 'Characters'
                    # token['data'] = '{{CODE}}'
                    _last_codeblock = 'BLOCK'
                    _codeblocked = 1
                    # this value gets set by our `code` handling
                    if token_stack__post and token_stack__post[-1].get('_md_code_compress'):
                        _discarded_pre = token_stack__post.pop()
                    lt = token_stack__post[-1]
                    if lt.get('_md_type') in _tts_md_newlines_single:
                        token_apply_prefix(lt, blockquote=_t_md_blockquote, codeblock=_codeblocked)
                    elif lt.get('_md_type') == tt_md_TokenNewlines:
                        _discarded_lines = token_stack__post.pop()
                        tok = _contextual_TokenNewlines(newlines=1, blockquoted=_t_md_blockquote, codeblocked=False)
                        token_stack__post.append(tok)
                        tok = _contextual_TokenNewlines(newlines=1, blockquoted=_t_md_blockquote, codeblocked=_codeblocked)
                        token_stack__post.append(tok)
                    else:
                        raise ValueError("DEBUG!!!! edge. this has never been triggered")
                else:
                    token['type'] = 'Characters'
        ...
        if _t_md not in _tts_md_filtered:
            ...
            if _t_md_blockquote or _codeblocked:
                token_apply_prefix(token, blockquote=_t_md_blockquote, codeblock=_codeblocked)
    ...
    for (token_idx, token) in enumerate(token_stack):
        ...
        if _t_md not in (tt_md_TokenHR, ):
            ...
            token['data'] = TokenHR()['data'].strip()  # replace this with a raw TokenHR's data
...
class MarkdownSerializer(HTMLSerializer):
    def serialize(self, domtree, encoding=None):
        ...
        """
        for (idx, token) in enumerate(super(MarkdownSerializer, self).serialize(domtree, encoding)):
            if token.startswith("\n") or (idx == 0):
